...
import { UserRowUpdateProps } from './props';
import { Btn, Indicator } from '@app/components';
import { useUpdateUserById } from '@hooks/user';

export const UserRowUpdate: React.FC<UserRowUpdateProps> = ({ id, rowName, label }): JSX.Element => {
  const { onOpen, onClose, isOpen } = useDisclosure();
  const toast = useToast();
  const [flag, setFlag] = useBoolean();
  const { user, error, isLoading, updateUser } = useUpdateUserById(id);
  ...
  useEffect(() => {
    ...
    if (user) toast({ title: 'User row update.', status: 'success', duration: 1000, isClosable: true });
  }, [error, user]);
