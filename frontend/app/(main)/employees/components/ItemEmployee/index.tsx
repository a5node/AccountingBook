...
import { HiMiniCog6Tooth, HiArrowPath } from 'react-icons/hi2';
...
import { ItemEmployerProps } from './props';
import { useUsersContext } from '@app/contexts';
import { useToggleStore } from '@store/toggles';

export const ItemEmployee: React.FC<ItemEmployerProps> = (): JSX.Element => {
  const { employee } = useUsersContext();
  const [isSetting, setIsSetting] = useBoolean();
  const {
    toggles: { reloadUser },
    setReloadUser,
  } = useToggleStore();
  const { user, isLoading, getUser, error } = useGetUserById(Number(employee));

  const reload = () => {
    getUser(Number(employee));
  };
