...
import { ItemPersonProps } from './props';
import {
  Indicator,
  BtnIcon,
  UserRowUpdate,
...
export const ItemPerson: React.FC<ItemPersonProps> = (): JSX.Element => {
  const { person } = useUsersContext();
  ...
  const { user, isLoading, getUser, error } = useGetUserById(Number(person));

  const reload = () => {
    getUser(Number(person));
  };
